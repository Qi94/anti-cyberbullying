...
gamma_list = zeros(1, experiment_times);
...
for n = 1:experiment_times
    gamma_list(1, n) = gamma_min;
    gamma_min = gamma_min + frequency;
end
...
for network_iter = 1:Number_of_Network
    ...
    for experiment_iter = 1:experiment_times
        ...
        % upper bound on instant increase rate of conducting anti-cyberbullying strategy at time
        gamma(1) = gamma_list(1, experiment_iter);
        gamma(2) = gamma_list(1, experiment_iter);
        % differential time
        dt = T/n;
...
filename = 'disucussion_experiment_2';
save(filename)
